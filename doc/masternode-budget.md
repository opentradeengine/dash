...
Digitalcoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
Format: ```mngovernance prepare proposal-name url payment-count block-start digitalcoin-address monthly-payment-digitalcoin```
...
Format: ```mngovernance submit proposal-name url payment-count block-start digitalcoin-address monthly-payment-digitalcoin fee-tx```
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single masternode (using digitalcoin.conf setup)
