...
// ***TODO*** ScanHash is not yet used in Digitalcoin
//
...
{
    LogPrintf("DigitalcoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Digitalcoin-miner");
    ...
    try {
        ...
        while (true) {
            ...
            LogPrintf("DigitalcoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("DigitalcoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("DigitalcoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}
